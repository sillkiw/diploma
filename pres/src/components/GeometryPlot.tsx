...
export function GeometryPlot({ P1, P2, geometry, lastStep, history }: Props) {
  ...
  if (geometry?.extras) {
   const { s1: s1raw, s2: s2raw, u, v } = geometry.extras;
  ...
  const s1 = s1raw.map((x) => x - mean(s1raw));
  const s2 = s2raw.map((x) => x - mean(s2raw));

  const cAbs1 = Math.max(1e-9, ...s1.map((val) => Math.abs(val)));
  const cAbs2 = Math.max(1e-9, ...s2.map((val) => Math.abs(val)));  
    traces.push({
      type: "scatter",
      mode: "markers",
